...
export default async function handler(req, res) {
  ...
  try {
    ...
    const rapydRes = await axios.post(
    ...
    const data = rapydRes.data;

    if (data.data && data.data.redirect_url) {
      res.setHeader('Access-Control-Allow-Origin', '*');
      return res.status(200).json({ redirect_url: data.data.redirect_url });
    } else {
      console.error('No redirect_url in Rapyd response:', data);
      return res.status(500).json({ error: 'No redirect URL received from Rapyd' });
    }
  } catch (error) {
    console.error('Error calling Rapyd /v1/checkout:', error.response?.data || error.message);
